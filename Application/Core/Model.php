...
class Model extends DBConnection
{
    ...
    public function __construct() {
        ...
        $tableNamePrefix = (!empty($defaultOptions['dbtable']['prefix'])) ? $defaultOptions['dbtable']['prefix'] : '';
        $tableNameSuffix = (!empty($defaultOptions['dbtable']['suffix'])) ? $defaultOptions['dbtable']['suffix'] : '';
        $this->tablePrimaryKey = (!empty($defaultOptions['dbtable']['primary_key'])) ?
            $defaultOptions['dbtable']['primary_key'] : 'id';
    ...
    {
        $sql = 'SELECT * FROM '.$this->tableName.' WHERE '.$this->tablePrimaryKey.'='.$id;
        $query = $this->connection->query($sql);
        if(!$query) {
            exit("Unable to execute the query ".$sql."<br>");
        }
